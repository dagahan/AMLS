...
def initialize_runtime(
    ...
    runtime_snapshot = RuntimeSnapshot(
        ...
        leader_problem_type_indices=posterior_summary["leader_problem_type_indices"],
    )

    logger.info(
        "Initialized entrance assessment runtime: node_count={}, feasible_state_count={}, initial_entropy={}, leader_state_probability={}",
        node_count,
        forest_artifact.feasible_state_count,
        forest_artifact.initial_entropy,
...
def apply_answer_step(
    ...
    max_questions: int,
    available_problem_type_ids: set[uuid.UUID],
    ...
    answered_problem_type_index = graph_artifact.index_by_id[answered_problem_type_id]
    previous_mastery_probability = float(
        runtime.marginal_probabilities[answered_problem_type_index]
    )
    ...
    previous_score = float(updated_node_scores[answered_problem_type_index])
    updated_node_scores[answered_problem_type_index] = previous_score + evidence_increment
    asked_problem_type_indices = tuple(
        dict.fromkeys(
            [*runtime.asked_problem_type_indices, answered_problem_type_index]
        )
    )
    ...
    updated_mastery_probability = float(
        updated_runtime.marginal_probabilities[answered_problem_type_index]
    )
    ...
    selection = _select_next_problem_type(
        graph_artifact=graph_artifact,
    ...
    logger.info(
        "Applied entrance assessment answer: problem_type_id={}, problem_type_index={}, outcome={}, difficulty_weight={}, evidence_increment={}, mastery_probability_before={}, mastery_probability_after={}, previous_score={}, updated_score={}, normalized_entropy={}, projection_confidence={}, frontier_confidence={}, exact_state_probability={}, next_problem_type_id={}, max_utility={}, stop={}, stop_reason={}",
        ...
        previous_score,
        float(updated_node_scores[answered_problem_type_index]),
...
def build_final_result(
    ...
    learned_problem_type_ids = tuple(
        ...
        for index in projection_snapshot.learned_problem_type_indices
    )
    inner_fringe_ids = tuple(
        graph_artifact.node_ids[index]
        for index in projection_snapshot.inner_fringe_problem_type_indices
    )
    outer_fringe_ids = tuple(
        graph_artifact.node_ids[index]
        for index in projection_snapshot.outer_fringe_problem_type_indices
    )

    logger.info(
        "Built entrance assessment final result: state_index={}, projection_confidence={}, frontier_confidence={}, exact_state_probability={}, learned_count={}, inner_fringe_count={}, outer_fringe_count={}, uncertain_count={}",
        runtime.leader_state_index,
        projection_snapshot.projection_confidence,
        projection_snapshot.frontier_confidence,
        runtime.leader_state_probability,
        len(learned_problem_type_ids),
        len(inner_fringe_ids),
        len(outer_fringe_ids),
        len(projection_snapshot.uncertain_problem_type_indices),
    )

    return FinalResult(
        state_index=runtime.leader_state_index or 1,
        state_probability=projection_snapshot.projection_confidence,
        learned_problem_type_ids=learned_problem_type_ids,
        inner_fringe_ids=inner_fringe_ids,
        outer_fringe_ids=outer_fringe_ids,
    )
...
def select_next_problem_type(
    ...
    return _select_next_problem_type(
        graph_artifact=graph_artifact,
        runtime=runtime,
        projection_snapshot=projection_snapshot,
        available_problem_type_ids=set(graph_artifact.node_ids),
    )
...
def should_stop(
    ...
    if asked_count >= max_questions:
        logger.info(
            "Entrance assessment stop by question cap: asked_count={}, max_questions={}",
            asked_count,
            max_questions,
        )
    ...
    if selection.problem_type_id is None:
        logger.info("Entrance assessment stop because no next problem is available")
        return True, "no_available_problem"
    ...
    if leader_probability_stop is not None:
        logger.debug(
            "Ignoring leader_probability_stop for stopping because projection confidence is now the product confidence: leader_probability_stop={}, exact_state_probability={}",
    ...
    if entropy_ready and utility_ready and projection_ready and frontier_ready:
        logger.info(
            "Entrance assessment stop by projection convergence: normalized_entropy={}, entropy_stop={}, max_utility={}, utility_stop={}, projection_confidence={}, projection_confidence_stop={}, frontier_confidence={}, frontier_confidence_stop={}, exact_state_probability={}",
            normalized_entropy,
            entropy_stop,
            selection.max_utility,
            utility_stop,
    ...
    logger.debug(
        "Entrance assessment continues: normalized_entropy={}, entropy_stop={}, max_utility={}, utility_stop={}, projection_confidence={}, projection_confidence_stop={}, frontier_confidence={}, frontier_confidence_stop={}, exact_state_probability={}, uncertain_count={}",
        normalized_entropy,
        entropy_stop,
        selection.max_utility,
        utility_stop,
...
def _build_runtime_snapshot(
    ...
    logger.debug(
        "Built entrance assessment runtime snapshot: asked_count={}, entropy={}, normalized_entropy={}, exact_state_probability={}, leader_problem_type_count={}",
        len(asked_problem_type_indices),
        ...
        runtime_snapshot.leader_state_probability,
        len(runtime_snapshot.leader_problem_type_indices),
    )
    return runtime_snapshot
...
def _build_posterior_summary(
    ...
    leader_problem_type_indices, leader_log_weight = _build_leader_state(
        forest_artifact=forest_artifact,
        node_scores=node_scores,
    )
    ...
    logger.debug(
        "Built entrance assessment posterior summary: log_partition={}, expected_log_weight={}, entropy={}, normalized_entropy={}, stable_state_index={}, raw_state_index={}, leader_state_probability={}",
        float(log_partition),
        expected_log_weight,
        current_entropy,
        normalized_entropy,
...
def _build_marginal_probabilities(
    ...
    for node_index in forest_artifact.preorder_indices:
        parent_index = forest_artifact.parent_by_index[node_index]
...
def _build_leader_state(
    ...
    for root_index in forest_artifact.root_indices:
        ...
        for child_index in forest_artifact.children_by_index[root_index]:
            restore_state(child_index, root_is_mastered)

    leader_problem_type_indices = tuple(
        index
        for index in forest_artifact.preorder_indices
        if bool(is_mastered_by_index[index])
    )

    logger.debug(
        "Built leader state: leader_problem_type_count={}, leader_log_weight={}",
        len(leader_problem_type_indices),
        leader_log_weight,
    )
    return leader_problem_type_indices, float(leader_log_weight)
...
def _build_state_rank(
    ...
    subtree_state_count_by_index = _build_subtree_state_counts(forest_artifact)
    learned_index_set = set(leader_problem_type_indices)
    ...
    def build_subtree_rank(node_index: int) -> int:
        ...
        for child_index in reversed(forest_artifact.children_by_index[node_index]):
    ...
    for root_index in reversed(forest_artifact.root_indices):
...
def _select_next_problem_type(
    graph_artifact: GraphArtifact,
    runtime: RuntimeSnapshot,
    projection_snapshot: ProjectionSnapshot,
    available_problem_type_ids: set[uuid.UUID],
) -> Selection:
    asked_problem_type_indices = set(runtime.asked_problem_type_indices)
    learned_problem_type_indices = set(projection_snapshot.learned_problem_type_indices)
    outer_fringe_problem_type_indices = set(
        projection_snapshot.outer_fringe_problem_type_indices
    )
    available_problem_type_indices = {
    ...
    selected_problem_type_index: int | None = None
    selected_problem_type_id: uuid.UUID | None = None
    ...
    candidate_groups = (
        tuple(
            ...
            if node_index in outer_fringe_problem_type_indices
        ),
    ...
    for candidate_group in candidate_groups:
        for node_index in candidate_group:
            if node_index not in available_problem_type_indices:
                continue

            if node_index in asked_problem_type_indices:
                continue
            ...
            if node_index in outer_fringe_problem_type_indices:
                ...
                prerequisite_index in learned_problem_type_indices
            ...
            if selection_score > selected_score:
                selected_problem_type_index = node_index
                selected_problem_type_id = graph_artifact.node_ids[node_index]
                selected_mastery_probability = mastery_probability
                selected_utility = utility
                selected_score = selection_score

        if selected_problem_type_id is not None:
            break

    logger.debug(
        "Selected next entrance assessment problem: problem_type_id={}, problem_type_index={}, mastery_probability={}, utility={}, selection_score={}, projected_outer_fringe_count={}",
        selected_problem_type_id,
        selected_problem_type_index,
        selected_mastery_probability,
        selected_utility,
        selected_score,
        len(projection_snapshot.outer_fringe_problem_type_indices),
    )

    return Selection(
        problem_type_id=selected_problem_type_id,
        problem_type_index=selected_problem_type_index,
...
def _build_projection_snapshot(
    ...
    learned_problem_type_indices_set: set[int] = set()
    uncertain_problem_type_indices: list[int] = []

    for node_index in graph_artifact.topological_order:
        mastery_probability = float(marginal_probabilities[node_index])
        prerequisites_are_learned = all(
            prerequisite_index in learned_problem_type_indices_set
            for prerequisite_index in graph_artifact.prerequisites_by_index[node_index]
        )

        if mastery_probability >= learned_mastery_probability and prerequisites_are_learned:
            learned_problem_type_indices_set.add(node_index)
            continue
        ...
        if (
            ...
            uncertain_problem_type_indices.append(node_index)

    learned_problem_type_indices = tuple(
        node_index
        for node_index in graph_artifact.topological_order
        if node_index in learned_problem_type_indices_set
    )
    inner_fringe_problem_type_indices = tuple(
        node_index
        for node_index in learned_problem_type_indices
        if any(
            child_index not in learned_problem_type_indices_set
            for child_index in graph_artifact.dependents_by_index[node_index]
        )
    )
    outer_fringe_problem_type_indices = tuple(
        node_index
        for node_index in graph_artifact.topological_order
        if node_index not in learned_problem_type_indices_set
        and all(
            prerequisite_index in learned_problem_type_indices_set
            for prerequisite_index in graph_artifact.prerequisites_by_index[node_index]
        )
    ...
    frontier_confidence_values = [
        float(marginal_probabilities[node_index])
        for node_index in inner_fringe_problem_type_indices
    ]
    frontier_confidence_values.extend(
        1.0 - float(marginal_probabilities[node_index])
        for node_index in outer_fringe_problem_type_indices
    )
    frontier_confidence = (
    ...
    projection_snapshot = ProjectionSnapshot(
        learned_problem_type_indices=learned_problem_type_indices,
        inner_fringe_problem_type_indices=inner_fringe_problem_type_indices,
        outer_fringe_problem_type_indices=outer_fringe_problem_type_indices,
        uncertain_problem_type_indices=tuple(uncertain_problem_type_indices),
    ...
    logger.debug(
        ...
        len(projection_snapshot.learned_problem_type_indices),
        len(projection_snapshot.inner_fringe_problem_type_indices),
        len(projection_snapshot.outer_fringe_problem_type_indices),
        len(projection_snapshot.uncertain_problem_type_indices),
        projection_snapshot.projection_confidence,
        projection_snapshot.frontier_confidence,
